...
export default function Ptoductdetails({ product, handleindexchange, selectedOptionIndex: propSelectedOptionIndex }: any) {
  ...
  const [selectedOptionIndex, setSelectedOptionIndex] = useState(propSelectedOptionIndex || 0);
  ...
  const selectedVariant = product.options?.[selectedOptionIndex];
  const paymentMethodsForSelectedVariant = selectedVariant?.payment_methods || [];
      ...
      <div className="flex items-baseline gap-2 md:gap-3">
        <span className="text-2xl md:text-3xl font-bold text-[#C33104]">
          {typeof product.options[selectedOptionIndex]?.final_price === "string"
            ...
            : Number(product.options[selectedOptionIndex]?.final_price || 0).toLocaleString("ar-SA")} ر.س
        </span>
        <span className="text-base md:text-lg text-muted-foreground line-through">
          {typeof product.options[selectedOptionIndex]?.original_price === "string"
            ? product.options[selectedOptionIndex].original_price
            : Number(product.options[selectedOptionIndex]?.original_price || 0).toLocaleString("ar-SA")} ر.س
        </span>
      </div>

      {/* Payment Options */}
      <div className="space-y-3 pt-4 md:pt-6">
        {paymentMethodsForSelectedVariant && paymentMethodsForSelectedVariant.length > 0 ? (
