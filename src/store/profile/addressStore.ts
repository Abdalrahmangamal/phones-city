...
interface AddressState {
  ...
  addAddress: (addressData: Omit<Address, 'id' | 'created_at'>) => Promise<void>;
  ...
  persist(
    (set, get) => ({
      ...
      addAddress: async (addressData) => {
        set({ loading: true, error: null });
        try {
          const response = await axiosClient.post('/api/v1/locations', addressData);
          if (response.data.status) {
      ...
      updateAddress: async (id, addressData) => {
        set({ loading: true, error: null });
        try {
          const response = await axiosClient.put(`/api/v1/locations/${id}`, addressData);
          if (response.data.status) {
