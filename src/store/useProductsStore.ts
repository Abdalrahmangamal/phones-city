...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  {/* الألوان لو موجودة */ }
  {
    ...
    className = {`w-[18px] h-[18px] rounded-full border-2 transition ${index === selectedIndex
...
<div className="flex items-center justify-between mt-[10px] w-full" >
  <div className="relative flex gap-3" >
    <p className="text-[#211C4D] md:text-[18px] text-[11px] font-[500] flex items-center gap-1" >
      { final?.toLocaleString() } < SaudiRiyalIcon className = "w-3.5 h-3.5" />
        </p>

{
  original > final && (
    <div className="relative" >
      <p className="text-[#211C4D] md:text-[18px] text-[11px] font-[500] opacity-50 line-through flex items-center gap-1" >
